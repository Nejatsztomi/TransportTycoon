...
namespace TransportTycoon.Test.Model
{
    ...
    {
        private class MockBuilding : BuildingBlocks
        {
            public MockBuilding(int height)
            {
                Height = height;
                BuildingEntity = Substitute.For<BuildingEntity>(1, 1);
            }
            public override BuildingEntity BuildingEntity
            {
                get;
                protected set;
            }
        ...
        public class GameTableAdvancedTests
        {
            ...
            public void StopEnvironment_WithUpwardBuildingBlock_AddsConnection()
            {
                // Arrange
                var table = CreateTestTable(3, 3, 2);
                var mockBuilding = new MockBuilding(2);
                ...
                Assert.Single(stop.Connections);
                Assert.Contains(mockBuilding, stop.Connections);
            }
        ...
        public class GameTableBridgeAndStopTests
        {
            ...
            {
                var table = CreateTestTable();
                var mockBlock = new MockBuilding(2); // Azonos magasság a HeightCheck miatt
                table.UpdateTable(0, 1, mockBlock); // Felül (x-1)

                bool result = table.StopEnvironment(1, 1);

                Assert.True(result);
                Assert.Contains(mockBlock, ((Stop)table[1, 1]).Connections!);
            }
            ...
            {
                var table = CreateTestTable();
                var mockBlock = new MockBuilding(2);
